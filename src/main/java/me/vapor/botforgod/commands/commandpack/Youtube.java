...
public class Youtube extends Command {
    ...
    public Youtube(NewMain instance){
        super(instance, "youtube", "youtube <start | verify> <channelid (only if start)>", new String[]{"yt"});
    ...
    public void onCommand(Message message, NewMain instance, String[] args) {
        ...
        for (Role role : user){
            if(role.getName().equalsIgnoreCase("YouTube")){
                new me.vapor.botforgod.utils.Message(message.getAuthor(), "Error!", instance.getConfig().getMessage("messageYoutubeAlreadyLinked", null, this), Color.RED, message.getChannel());
                return;
            }
        }
        if(args[0].equalsIgnoreCase("start")){
            ...
            if(!args[1].matches(".{24}")){
                new me.vapor.botforgod.utils.Message(message.getAuthor(), "Error!", instance.getConfig().getMessage("messageYoutubeNotValidId", null, this), Color.RED, message.getChannel());
                return;
            }

            for (State state : users){
                if(state.getUserid().equals(message.getAuthor().getIdAsString())){
                    new me.vapor.botforgod.utils.Message(message.getAuthor(), "Error!", instance.getConfig().getMessage("messageYoutubeAlreadyLinking", null, this), Color.RED, message.getChannel());
                    return;
                }
            }
        ...
        if(args[0].equalsIgnoreCase("verify")){
            ...
            for (State record : users){
                if(record.getUserid().equals(message.getAuthor().getIdAsString())){
                    if(work(record)){
                        new me.vapor.botforgod.utils.Message("YouTube Verfiy",
                        ...
                        new me.vapor.botforgod.utils.Message(message.getAuthor(), "Error!", instance.getConfig().getMessage("messageYoutubeInvalid", null, this), Color.RED, message.getChannel());
                        state = record;
                    }
                }
            }
            new me.vapor.botforgod.utils.Message(message.getAuthor(), "Error!", instance.getConfig().getMessage("messageYoutubeProcessNotStarted", null, this), Color.RED, message.getChannel());
            if(state != null){
                users.remove(state);
            }
        }
