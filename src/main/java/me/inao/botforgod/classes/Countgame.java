...
public class Countgame {
    ...
    public Countgame(NewMain instance, long num, long max, TextChannel channel){
        ...
        new me.inao.botforgod.utils.Message("CountGame Created!", instance.getConfig().getMessage("messageCountgameCreate", new String[][]{
    ...
    public void onSend(Message message){
        ...
        if(message.getAuthor().canCreateChannelsOnServer() && message.getContent().equalsIgnoreCase("close")){
            new me.inao.botforgod.utils.Message("CountGame complete!", instance.getConfig().getMessage("messageCountgameClosed", new String[][]{{"%user%", message.getUserAuthor().get().getDiscriminatedName()}}, null), Color.yellow, channel);
        ...
        if(num == max){
            new me.inao.botforgod.utils.Message("CountGame complete!", instance.getConfig().getMessage("messageCountgameComplete", new String[][]{{"%user%", message.getUserAuthor().get().getDiscriminatedName()}}, null), Color.yellow, channel);
            instance.setCountgame(null);
            channel.asServerTextChannel().get().updateTopic("");
            return;
        }
